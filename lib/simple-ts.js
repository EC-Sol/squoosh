...
export default function simpleTS(mainPath, { noBuild, watch } = {}) {
  ...
  async function tsBuild(rollupContext) {
    ...
    try {
      execSync(`${tscPath} ${args.join(' ')}`, { stdio: 'inherit' });
      built = true;
    } catch (e) {
    ...
    if (watch) {
      // ... ignore watch implementation for now or use spawn for watch
    }
  }
  ...
  return {
    ...
    resolveId(id, importer) {
      ...
      if (
        ...
        tsResolve.resolvedModule.resolvedFileName.includes('node_modules')
      ) {
        return null;
      }
    ...
    async load(id) {
      ...
      try {
        const content = await fsp.readFile(newId, { encoding: 'utf8' });
        return content;
      } catch (e) {
        console.error(`SimpleTS failed to load: ${newId}`, e);
        throw e;
      }
    },
  };
