  ...
  return {
    ...
    load(id) {
      ...
      return [
        ...
        `export const imports = import.meta.${importsPlaceholder + exportCounter
        };`,
        `export const allSrc = import.meta.${allSrcPlaceholder + exportCounter
        };`,
      ].join('\n');
    },
    ...
    resolveImportMeta(property, { moduleId, format }) {
      ...
      if (property.startsWith(allSrcPlaceholder)) {
        ...
        const inlineDefines = [
          ...allModules.map(
            (item) =>
              `self.nextDefineUri=location.origin+${resolveFileUrl(item)};${item.code
              }`,
          ),
          'self.nextDefineUri=""',
        ];
