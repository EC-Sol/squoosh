...
import { ToggleAliasingIcon, ToggleAliasingActiveIcon, ToggleBackgroundIcon, AddIcon, RemoveIcon, ToggleBackgroundActiveIcon, RotateIcon, } from '../../icons';
...
export default class Output extends Component {
    ...
    render({ mobileView, leftImgContain, rightImgContain, source }, { scale, editingScale, altBackground, aliasing }) {
        const leftDraw = this.leftDrawable();
        const rightDraw = this.rightDrawable();
        ...
        return (_jsxs(Fragment, { children: [_jsx("div", { class: `${style.output} ${altBackground ? style.altBackground : ''}`, children: _jsxs("two-up", { "legacy-clip-compat": true, class: style.twoUp, orientation: mobileView ? 'vertical' : 'horizontal', 
                        ...
                        isSafari ? undefined : this.onRetargetableEvent, onMouseDownCapture: this.onRetargetableEvent, onWheelCapture: this.onRetargetableEvent, children: [_jsx("pinch-zoom", { class: style.pinchZoom, onChange: this.onPinchZoomLeftChange, ref: linkRef(this, 'pinchZoomLeft'), children: _jsx("canvas", { class: `${style.pinchTarget} ${aliasing ? style.pixelated : ''}`, ref: linkRef(this, 'canvasLeft'), width: leftDraw && leftDraw.width, height: leftDraw && leftDraw.height, style: {
                                        width: originalImage ? originalImage.width : '',
                                        height: originalImage ? originalImage.height : '',
                                        objectFit: leftImgContain ? 'contain' : '',
                                    } }) }), _jsx("pinch-zoom", { class: style.pinchZoom, ref: linkRef(this, 'pinchZoomRight'), children: _jsx("canvas", { class: `${style.pinchTarget} ${aliasing ? style.pixelated : ''}`, ref: linkRef(this, 'canvasRight'), width: rightDraw && rightDraw.width, height: rightDraw && rightDraw.height, style: {
                                        width: originalImage ? originalImage.width : '',
                                        height: originalImage ? originalImage.height : '',
                                        objectFit: rightImgContain ? 'contain' : '',
                                    } }) })] }) }), _jsxs("div", { class: style.controls, children: [_jsxs("div", { class: style.buttonGroup, children: [_jsx("button", { class: style.firstButton, onClick: this.zoomOut, children: _jsx(RemoveIcon, {}) }), editingScale ? (_jsx("input", { type: "number", step: "1", min: "1", max: "1000000", ref: linkRef(this, 'scaleInput'), class: style.zoom, value: Math.round(scale * 100), onInput: this.onScaleInputChanged, onBlur: this.onScaleInputBlur })) : (_jsxs("span", { class: style.zoom, tabIndex: 0, onFocus: this.onScaleValueFocus, children: [_jsx("span", { class: style.zoomValue, children: Math.round(scale * 100) }), "%"] })), _jsx("button", { class: style.lastButton, onClick: this.zoomIn, children: _jsx(AddIcon, {}) })] }), _jsxs("div", { class: style.buttonGroup, children: [_jsx("button", { class: style.firstButton, onClick: this.onRotateClick, title: "Rotate", children: _jsx(RotateIcon, {}) }), !isSafari && (_jsx("button", { class: style.button, onClick: this.toggleAliasing, title: "Toggle smoothing", children: aliasing ? (_jsx(ToggleAliasingActiveIcon, {})) : (_jsx(ToggleAliasingIcon, {})) })), _jsx("button", { class: style.lastButton, onClick: this.toggleBackground, title: "Toggle background", children: altBackground ? (_jsx(ToggleBackgroundActiveIcon, {})) : (_jsx(ToggleBackgroundIcon, {})) })] })] })] }));
    }
}
